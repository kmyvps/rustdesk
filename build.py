...
hbb_name = 'kmdesk' + ('.exe' if windows else '')
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/etc/kmdesk/')
    ...
    system2('rm tmpdeb/usr/bin/kmdesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/lib/kmdesk/')
    system2(
    ...
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/kmdesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/kmdesk/')
    system2(
        'cp ../res/pam.d/kmdesk.debian tmpdeb/etc/pam.d/kmdesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/kmdesk/files/polkit && chmod a+x tmpdeb/usr/share/kmdesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file('usr/share/kmdesk/files/systemd/kmdesk.service')
    system2('dpkg-deb -b tmpdeb kmdesk.deb;')
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/kmdesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/lib/kmdesk/')
    system2(
        'cp ../res/kmdesk.service tmpdeb/usr/share/kmdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/kmdesk.png')
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibkmdesk.dylib target/release/libkmdesk.dylib")
    os.chdir('flutter')
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libkmdesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/libkmdesk.dll"):
